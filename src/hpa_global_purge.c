...
hpa_global_purge(tsdn_t *tsdn, hpa_global_purge_t *hpa_purge, size_t nshards,
	...
	for(size_t i = 0; i<nshards; ++i) {
		...
		if (!to_hugify_only && cur->should_purge) {
			...
			purged += hpa_try_purge_then_hugify(
				tsdn, cur->shard, /*target*/ SIZE_T_MAX, /* minbit */ 0,
				&cur->stats, &cur->purge_bitmap[0],
				&cur->hugify_blocked_by_dirty, &cur->should_purge);
		} else {
	...
	pszind_t minbit = PSSET_NPURGE_LISTS - 1;
	while (purged < npurge && !hpa_gpheap_empty(&pheap)) {
		hpa_purge_analytics_t *first = hpa_gpheap_first(&pheap);
		if (fb_get(first->purge_bitmap, PSSET_NPURGE_LISTS, minbit)) {
			size_t target = npurge - purged;
			hpa_purge_analytics_t *cur = hpa_gpheap_remove_first(&pheap);
			size_t ndirty =
			    hpa_try_purge_then_hugify(tsdn, cur->shard, target, minbit,
						      &cur->stats, &cur->purge_bitmap[0],
						      &cur->hugify_blocked_by_dirty,
						      &cur->should_purge);
			purged += ndirty;
			if (cur->should_purge) {
				assert(purged < npurge ||
				       !fb_get(cur->purge_bitmap, PSSET_NPURGE_LISTS, minbit));
				hpa_gpheap_insert(&pheap, cur);
			}
		} else if (minbit > 0) {
			minbit--;
