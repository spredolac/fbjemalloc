...
static void
test_init_shard_stats(hpa_purge_analytics_t *dest, bool hug_blocked,
		      size_t ndirty, size_t nactive) {
	dest->shard = NULL;
	dest->hugify_blocked_by_dirty = hug_blocked;
	dest->stats.ndirty = ndirty;
	dest->stats.nactive = nactive;
	dest->stats.npageslabs = 0;
	dest->should_purge = true;
}

static void
test_init_shard_stats_full(hpa_purge_analytics_t *dest, void *fake_shard,
			   bool hug_blocked, size_t ndirty) {
	test_init_shard_stats(dest, hug_blocked, ndirty, 0);
...
TEST_BEGIN(test_max_over_period) {
	...
	test_init_shard_stats(&analytics[0], false, 20, 100);
	test_init_shard_stats(&analytics[1], false, 30, 200);
	...
	test_init_shard_stats(&analytics[1], false, 40, 300);
	hpa_global_purge_tick(&hpa_purge, NSHARDS, &after_sec);
	expect_zu_eq(hpa_purge.prior_active_max, 0, "");
	expect_zu_eq(hpa_purge.current_active_max, 400, "");

	test_init_shard_stats(&analytics[1], false, 40, 100);
	/* Advance one second and update the stats */
	...
	test_init_shard_stats(&analytics[1], false, 0, 10);
	/* Advance one second and update the stats */
...
TEST_BEGIN(test_calc_target) {
	 ...
	 */
	test_init_shard_stats(&analytics[1], false, 140, 100);
	size_t tgt = hpa_global_purge_tick(&hpa_purge, NSHARDS, &period1);
	expect_zu_eq(hpa_purge.prior_active_max, 300, "");
	expect_zu_eq(hpa_purge.current_active_max, 200, "");
	expect_zu_eq(tgt, 60,"Start the purge after 1st period");

	/*
	 * Have current active 110, prior active max is 200, and 120 dirty.
	 * Target should be 120 + 110 - 200 = 30.  
	 */
        test_init_shard_stats(&analytics[1], false, 100, 10);
	tgt = hpa_global_purge_tick(&hpa_purge, NSHARDS, &period2);
        expect_zu_eq(hpa_purge.prior_active_max, 200, "");
	expect_zu_eq(hpa_purge.current_active_max, 110, "");
        expect_zu_eq(tgt, 30, "");
}
...
extern size_t (*hpa_try_purge_then_hugify_hook)(tsdn_t *, hpa_shard_t *,
						size_t, bool *);
...
TEST_BEGIN(test_global_purge) {
	...
	hpa_purge_analytics_t *analytics = hpa_purge.analytics;
	test_init_shard_stats_full(&analytics[0], shard2, true, 20);
	test_init_shard_stats_full(&analytics[1], shard4, false, 30);
	test_init_shard_stats_full(&analytics[2], shard3, false, 50);
        test_init_shard_stats_full(&analytics[3], shard1, true, 20);

	hpa_global_purge(tsdn, &hpa_purge, NSHARDS, target, false);
